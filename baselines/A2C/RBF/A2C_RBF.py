# This is the baseline experiment code about the A2C with the RBF kernel
...
def export_history_csv(
    ...
    """
    Save dict-of-lists history to CSV.
    - history: keys -> list[float] (same length)
    - add_time_index: add 't' column as step index
    """
...
def _nash_team_reward(mg_r: float, dg_r: float, cfg) -> float:
    """Nash bargaining team objective.

    r_team = log(softplus((r_mg-ref_mg)/scale_mg)+eps) + log(softplus((r_dg-ref_dg)/scale_dg)+eps)
    """
...
# Actor/Critic networks (classical)
...
class ClassicalKernelRegularizer:
    """RBF (Gaussian) kernel for critic smoothness regularization.
    Replaces the previous quantum kernel implementation for completely classical execution.
    """
    ...
    def __init__(self, feature_dim: int, gamma: float = 1.0):
        ...
        self.enabled = True
        # When the code is wrong, print some warnings.
        print("We turn on the Classical Kernel！！\n")

    def matrix(self, z: torch.Tensor) -> torch.Tensor:
        # z detach to emulate the identical non-differentiable behavior of the quantum kernel version
        z_det = z.detach()
...
# Plot helpers (per-update logging)
...
def plot_training_curves(log: TDict[str, List[float]], save_path: str = "training_curves.png"):
    ...
    axes[1].plot(np.arange(len(log.get("critic_loss", []))), log.get("critic_loss", []), label="critic_total")
    if len(log.get("critic_td_loss", [])) > 0:
        axes[1].plot(np.arange(len(log.get("critic_td_loss", []))), log.get("critic_td_loss", []), label="critic_td")
...
class RolloutStep:
    ...
    value: np.ndarray  # [1] (filled later)
...
class CTDEConfig:
    ...
    # Classical-kernel critic regularization
    # You can change the super parameters at here.
    use_classical_kernel: bool = True
    kernel_reg_coef: float = 1e-3
    kernel_latent_dim: int = 4
    kernel_gamma: float = 1.0
    kernel_max_samples: int = 32
    kernel_eval_every: int = 5

# Trainer (4 MG, independent actors)
class CTDE_A2C_Trainer4MG:
    def __init__(self, mg_envs: List[Any], dg_env: Any, cfg: CTDEConfig):
        ...
        if not self._dg_two_phase:
            print("[warn] DG env does NOT have apply_price_action/settle. "
                  "Trainer will fall back to step()+set_grid_power scheme (1-step lag).")
        ...
        for _t in range(cfg.horizon):
            ...
            if self._dg_two_phase:
                try:
                    ...
                    print(f"[warn] dg_env.apply_price_action failed: {e}. Falling back to step().")
            ...
            if self._dg_two_phase:
                try:
                    ...
                    print(f"[warn] dg_env.settle failed: {e}. Falling back to lag injection.")
                    if hasattr(self.dg_env, "set_grid_power"):
    ...
    def _compute_kernel_smoothness_loss(self, s_global_t: torch.Tensor, v_pred: torch.Tensor) -> torch.Tensor:
        ...
        if (self._update_idx % max(cfg.kernel_eval_every, 1)) != 0:
            return torch.zeros((), device=s_global_t.device, dtype=v_pred.dtype)
    ...
    def update(self, batch: RolloutBatch, last_value: float):
        ...
        critic_kernel_loss = self._compute_kernel_smoothness_loss(s_global_t, v_pred) * cfg.kernel_reg_coef
        ...
        self.log["critic_td_loss"].append(float(critic_td_loss.item()))
        self.log["critic_kernel_loss"].append(float(critic_kernel_loss.item()))
        print("------Critic_kernel_loss=", float(critic_kernel_loss.item()))
